...
export default function Home() {
  ...
  const generateTarget = useCallback(grid => {
    ...
    return operation === 'add' ? first.value * second.value + third.value : first.value * second.value - third.value;
  }, []);
  ...
  const handleCellClick = cell => {
    ...
    const newGrid = grid.map(row => row.map(c => c.row === cell.row && c.col === cell.col ? {
      ...c,
      isSelected: !c.isSelected
    } : c));
    let newSelectedCells;
    if (cell.isSelected) {
      newSelectedCells = selectedCells.filter(selected => !(selected.row === cell.row && selected.col === cell.col));
      ...
      newSelectedCells = [...selectedCells.slice(1), {
        ...cell,
        isSelected: true
      }];
    }
  ...
  const checkSelection = selectedCells => {
    ...
    const isHorizontal = first.row === second.row && second.row === third.row && Math.abs(first.col - second.col) === 1 && Math.abs(second.col - third.col) === 1;
    const isVertical = first.col === second.col && second.col === third.col && Math.abs(first.row - second.row) === 1 && Math.abs(second.row - third.row) === 1;
    const isDiagonal1 = first.row + 2 === third.row && first.col + 2 === third.col || first.row + 2 === third.row && first.col - 2 === third.col;
    const isDiagonal2 = first.row - 2 === third.row && first.col + 2 === third.col || first.row - 2 === third.row && first.col - 2 === third.col;
    ...
    if (addResult === gameState.target || subtractResult === gameState.target) {
      ...
      setGameState(prev => {
        ...
        return {
          ...
          message: endGame ? `Game Over! Final time: ${totalTime}` : 'Wrong combination! Try again.',
    ...
    children: /*#__PURE__*/_jsxs("div", {
      ...
      children: [/*#__PURE__*/_jsxs("div", {
        ...
        children: [/*#__PURE__*/_jsx("h1", {
          ...
          children: "Math Grid Puzzle"
        }), /*#__PURE__*/_jsx("p", {
          className: "text-gray-600",
          children: "Find 3 numbers in a straight line where (1st \xD7 2nd) \xB1 3rd = Target"
        })]
      }), /*#__PURE__*/_jsx("div", {
        ...
        children: /*#__PURE__*/_jsxs("div", {
          ...
          children: [/*#__PURE__*/_jsxs("button", {
            ...
            className: `px-4 py-2 rounded-lg font-semibold transition-colors flex items-center gap-2 ${gameState.gameMode === 'timed' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`,
            ...
            className: `px-4 py-2 rounded-lg font-semibold transition-colors flex items-center gap-2 ${gameState.gameMode === 'guesses' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`,
            disabled: gameState.isPlaying,
